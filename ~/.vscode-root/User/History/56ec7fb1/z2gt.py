...
from .models import (Evento,Instituicao,Capacidade,Unidade, UnidadeCapacidade,Periodo,
                     PeriodoDetalhe, HoraIntervalo, PeriodoIntervalo, Pessoa, PessoaViagem ,AreaTecnologica, Infraestrutura, Turma, Aula, AulaSala,
                     AulaProfessor)
from .serializers import (EventoSerializer,InstituicaoSerializer,CapacidadeSerializer, 
                          ...
                          PeriodoDetalheSerializer, HoraIntervaloSerialilzer, PeriodoIntervaloSerializer, PessoaSerializer,PessoaViagemSerializer, AreaTecnologicaSerializer, 
...
class PessoaViagemAPIView(APIView):
    '''APIview para realizar CRUD para a tabela pessoas em motivos de viagens'''
    def get(self, request):
        pessoas = PessoaViagem.objects.all()
        serializer = PessoaSerializer(pessoas, many=True)
        return Response(serializer.data)

    def post(self, request):
        serializer = PessoaViagemSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=201)
        return Response(serializer.errors, status=400)
    ...
    def put(self, request, pk, format=None):
        pessoa = PessoaViagem.objects.get(pk=pk)
        serializer = PessoaViagemSerializer(pessoa, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
