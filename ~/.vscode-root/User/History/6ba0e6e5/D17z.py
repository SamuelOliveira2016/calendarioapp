...
class Periodo(models.Model):
    ...
    periodo = models.JSONField(default=list)
    ...
    diasSemana = models.JSONField(default=list)
    tempo_aula_min = models.IntegerField()
    hora_intervalo = models.IntegerField()
...
class SemanaForm(forms.ModelForm):
    ...
    def clean_diasSemana(self):
        # Convertendo os dias selecionados para JSON antes de salvar
        dias = self.cleaned_data['diasSemana']
        return dias
    
class PeriodoForm(forms.ModelForm):
    periodo = forms.MultipleChoiceField(
        choices=Periodo.PERIODO_CHOICE, 
        required=False, 
        widget=forms.CheckboxSelectMultiple
    )
    ...
    def clean_periodos(self):
        # Convertendo os dias selecionados para JSON antes de salvar
        periodos = self.cleaned_data['periodo']
        return periodos
...
class HoraIntervalo(models.Model):
    '''Tabela para apresentar a duração de cada intervalo entre as aulas e jornadas de trabalho.'''
    horaInicio = models.TimeField()
    horaFim = models.TimeField()
    periodo = models.IntegerField()

class PeriodoIntervalo(models.Model):
    '''Tabela que auxilia na apresentação dos resultados das tabelas periodo e hora intervalo'''
    idPeriodo = models.ForeignKey(Periodo, on_delete=models.CASCADE)
    idIntervalo = models.ForeignKey(HoraIntervalo,on_delete=models.CASCADE)
    periodo = models.IntegerField()
